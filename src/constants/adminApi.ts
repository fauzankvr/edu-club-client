export const ADMIN_LOGIN_API = "/admin/login";
export const ADMIN_LOGOUT_API = "/admin/logout";

export const ADMIN_BLOCK_STUDENT_API = "/admin/blockStudent";
export const ADMIN_FIND_ALL_STUDENTS_API = "/admin/findAllStudent";
export const ADMIN_GET_ALL_TEACHERS_API = "/admin/findAllTeachers";
export const ADMIN_BLOCK_TEACHER_API = "/admin/blockTeacher";
export const ADMIN_APPROVE_TEACHER_API = "/admin/teachers/approve";


export const ADMIN_CATEGORY = "/admin/category";
export const ADMIN_CATEGORY_UPDATE_API = "/admin/category/update";

export const ADMIN_LANGUAGE = "/admin/language";
export const ADMIN_LANGUAGE_UPDATE_API = "/admin/language/update";

export const ADMIN_PAYOUTS_API = "/admin/payouts";
export const ADMIN_PAYOUT_API = "/admin/payout";

export const ADMIN_PLANS_API = "/admin/plans";
export const ADMIN_PLAN_API = "/admin/plan";

export const ADMIN_DASHBOARD_API = "/admin/dashboard";
export const ADMIN_REPORT_API = "/admin/report";

export const ADMIN_COURSES_API = "/admin/courses";
export const ADMIN_BLOCK_COURSE_API = "/admin/course";
